...
The three fields agree with each other, which is the whole point of the DTO. Same-basename classes are aliased apart — two classes both named `User` come back as separate aliases, and the property types spell those aliases rather than the bare name. An `EnumResource::make()` property arrives already wrapped as `AsEnum<typeof X>` beside the value import that wrapper needs. And nothing is imported that no property type names. A name counts wherever its token appears, so a type that spells an imported name only inside a string, template text or a comment, such as a method-level `#[TsCasts]` of `"'User' | 'Admin'"`, keeps that import, unused, which TypeScript reports only under `noUnusedLocals`. Render all three verbatim and the module compiles.
...
Every pattern documented in [API Resources](./api-resources.md) resolves identically here — the `when()` conditional-method family, `EnumResource::make()`, nested and collection resources, `merge()` / `mergeWhen()`, and relation filters (`$this->author->only([...])`) all produce the same properties, imports, and optionality a full publish would. The exceptions — a `morphTo` union, a `$wrap = null` collection, class- and model-level `#[TsCasts]`, and an interpolated key's filled index signature — are [below](#what-it-cannot-do).
...
**Class- and model-level `#[TsCasts]` are a publish step.** `analyze()` applies a method-level `#[TsCasts]` only. `ts:publish` then lays a resource's class-level `#[TsCasts]` and its backing model's over the analysis, types and optionality both, so on a `CommentResource` whose class-level casts name `flagged_at` and `metadata`, `analyze()` returns `flagged_at: string | null` and `metadata: unknown[] | null` where the published interface reads `flagged_at?: string | null` and `metadata: Record<string, unknown>`. A publish run also gates a naming-convention `toResource()` guess on the guessed resource being published; outside one every existing resource class passes, so `analyze()` can name a resource that a publish excluding it types `unknown`.

**An index signature's `@return` fill is checked only against the keys the method returns.** `ts:publish` also checks it against a resource's class-level `#[TsCasts]` keys and its backing model's `#[TsCasts]`, or a broadcast event's class-level ones, and against the extends clause, and puts the fill back to `unknown | undefined` wherever they could conflict — always, under an extends clause (see [API Resources § Interpolated Keys](./api-resources.md#interpolated-keys)). `analyze()` applies neither, so on a `#[TsExtends]` resource it returns ``[key: `${string}_tag`]: string | undefined`` where the published interface reads `unknown | undefined`.

**`unknown` is an honest floor, not a bug.** Every pattern this page documents is one the analyzer specifically recognizes; anything else — an expression it can't trace, a reassigned local — degrades to `unknown` rather than guessing. Two cases don't widen to `unknown`: a conditional default it can't type leaves the value's own type standing, and a ternary or `?:` arm it can't type is left out, so `$cond ? $untypable : null` publishes `null`. See [API Resources § Local Variables and Narrowing](./api-resources.md#local-variables-and-narrowing) for what that looks like from the resource side.
