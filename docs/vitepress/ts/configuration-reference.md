...
The `*_class` keys swap a class in the publishing pipeline. See [Customizing the Pipeline](./customizing-the-pipeline.md) for what each class must implement.
...
| `output_to_files`             | `bool`   | `true`                    | Write generated files to disk. When `false`, no files are written and the post-migration republish is off                                                                               |
...
`form_requests.analyzer_class` has no line in the published config file. Add the key to the `form_requests` block to point it at your own class. The same class also types [`$request->validated('key')`](./form-requests.md#reading-a-single-validated-field) in a route or page prop, so swapping it changes those types too.
...
| `broadcast_events.echo_augmentation.echo_package`     | `?string` | `null`                                       | npm package to augment. When `null`, the first of `@laravel/echo-vue`, `@laravel/echo-react`, or `@laravel/echo-svelte` in `package.json`, otherwise `@laravel/echo` |
...
| `inertia.component_casing`        | `string`  | `'camel'`             | Casing style for derived page-prop export names, used by the per-route page props. See [Routing](./routing.md) |
