...
The limit: a precise annotation still wins outright and is read *before* the body, so this never overrides an accurate `@return Attribute<Get, Set>`. The body step inherits the engine's limits, so what stays vague is whatever the engine itself can't resolve: an empty `[]` literal (it carries no element information, so `empty_list` keeps its `: array` signature), loop-built keys (`dynamic_totals` above), and a call whose own return the engine can't type. Two accessors that read each other both degrade to `unknown` rather than recursing forever. The body step also declines three results the engine itself resolves, falling back to the annotation as it does for a body it can't follow, so an untyped getter publishes `unknown`: a body left as only `null` once a `??` or ternary arm it can't type is dropped (`fn ($value, array $attributes) => $attributes['title'] ?? null`, whose real value is the title), a body naming two same-named classes or enums under one name (`$this->status ?? $this->crmAuthor?->status`), and a body returning a resource.
...
- **Write-only mutators** — `Attribute::make(set: ...)` or `Attribute::set(...)`, with no getter — resolve in this order: (1) the method's own `@return Attribute<Get, Set>` docblock, when the `Get` type is present and is neither vague nor `never`; (2) a same-named database column's database type, if one exists; (3) otherwise the mutator is omitted from the interface entirely, rather than emitted as `unknown`, unless a class-level `@property` tag types it. `Attribute<never, string>` (or `?never`) is the conventional way to write a mutator with no getter: its `never` records that no getter exists, not what reading returns, so a column behind it publishes its own database type — `subject: string`, not `subject: never`. Step (2) skips the column's cast: Laravel reports a column with a mutator as cast by that mutator, so an enum, `integer` or `boolean` cast on it is not applied, and an old-style `setFooAttribute()` does the same. Give such a column a specific `Get` (`Attribute<Role|null, string>` publishes `RoleType | null`) or a `#[TsCasts]` entry.
