...
| `$this->relation->only([...])`/`->except([...])` losing the related model's own `#[TsCasts]`/`@property` refinements | Nothing extra — automatic whenever the relation resolves to a single model and every filtered key is a real database column                                             | `Pick<Model, 'a' \| 'b'>` / `Omit<Model, ...>` referencing the model's own generated interface — see [API Resources § Relation Filters](./api-resources.md#relation-filters)                           |
