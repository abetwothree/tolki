...
As mentioned in the [Enum Utilities Installation](./index.md) page, the `@tolki/ts/enum` package is not meant to be used as standalone package as it works with the [Laravel TypeScript Publisher](https://github.com/abetwothree/laravel-ts-publish) package to transform PHP enums into TypeScript enums. The functions below are being listed out for reference, not necessarily for direct use.
...
```javascript
import { cases } from "@tolki/ts/enum";
...
```javascript
import { defineEnum } from "@tolki/ts/enum";
...
```javascript
import { from } from "@tolki/ts/enum";
import { Status } from "@js/types/enums";
...
```javascript
import { tryFrom } from "@tolki/ts/enum";
import { Status } from "@js/types/enums";
