...
describe("Path Functions", () => {
    ...
    describe("getRaw", () => {
        ...
        it("returns the whole array for null/undefined keys", () => {
            expect(Path.getRaw(testArray, null)).toEqual({ found: true, value: testArray });
            expect(Path.getRaw(testArray, undefined)).toEqual({ found: true, value: testArray });
        });

        it("gets values by numeric index", () => {
            expect(Path.getRaw(testArray, 0)).toEqual({ found: true, value: ['a', 'b'] });
            expect(Path.getRaw(testArray, 2)).toEqual({ found: true, value: 'e' });
        ...
        it("gets values by dot notation", () => {
            expect(Path.getRaw(testArray, "0.1")).toEqual({ found: true, value: 'b' });
            expect(Path.getRaw(testArray, "1.0")).toEqual({ found: true, value: 'c' });
    ...
    describe("forgetKeys", () => {
        ...
        it("removes multiple numeric keys", () => {
            expect(Path.forgetKeys(['a', 'b', 'c', 'd'], [1, 3])).toEqual(['a', 'c']);
        ...
        it("removes multiple mixed keys", () => {
            ...
            expect(Path.forgetKeys(data, [2, "0.1"])).toEqual([['x'], ['a', 'b']]);
        });
        ...
        it("handles complex nested removals", () => {
            ...
            expect(Path.forgetKeys(data, "0.0.1")).toEqual([[['deep1'], ['deep3']], [['deep4']]]);
        });
        ...
        it("handles edge cases in nested removal operations", () => {
            ...
            expect(Path.forgetKeys(data3, ["0.0", "1.0"])).toEqual(['not-array', ['c']]);
        });
    });

    describe("setImmutable", () => {
        it("returns value as array when key is null", () => {
            expect(Path.setImmutable(['a', 'b'], null, 'replacement')).toEqual('replacement');
        });
        ...
        it("sets value at numeric index", () => {
            expect(Path.setImmutable(['a', 'b', 'c'], 1, 'x')).toEqual(['a', 'x', 'c']);
            expect(Path.setImmutable(['a', 'b'], 2, 'c')).toEqual(['a', 'b', 'c']); // Extends array
        });
        ...
        it("sets nested values with dot notation", () => {
            expect(Path.setImmutable([['a', 'b']], "0.1", 'x')).toEqual([['a', 'x']]);
            expect(Path.setImmutable([['a']], "0.1", 'b')).toEqual([['a', 'b']]);
        });

        it("creates nested structures as needed", () => {
            expect(Path.setImmutable([], "0.0.0", 'deep')).toEqual([[['deep']]]);
            expect(Path.setImmutable([null], "0.0", 'value')).toEqual([['value']]);
        });

        it("handles invalid path segments", () => {
            const data = ['a', 'b'];
            expect(Path.setImmutable(data, "invalid", 'x')).toEqual(['a', 'b']);
            expect(Path.setImmutable(data, "1.invalid", 'x')).toEqual(['a', 'b']);
        });

        it("preserves existing structure when setting nested values", () => {
            const data = [['a', 'b'], ['c', 'd']];
            const result = Path.setImmutable(data, "0.1", 'x');
            expect(result).toEqual([['a', 'x'], ['c', 'd']]);
    ...
    describe("pushWithPath", () => {
        it("pushes to root array when key is null", () => {
            const data = ['a'];
            expect(Path.pushWithPath(data, null, 'b', 'c')).toEqual(['a', 'b', 'c']);
        });

        it("returns values array when data is not array and key is null", () => {
            expect(Path.pushWithPath("not-array", null, 'a', 'b')).toEqual(['a', 'b']);
        });

        it("creates nested structure for non-accessible data", () => {
            expect(Path.pushWithPath("not-array", "0", 'value')).toEqual(['value']);
        ...
        it("extends arrays when setting at higher indices", () => {
            const data = [[]];
            expect(Path.pushWithPath(data, "0", 'a', 'b')).toEqual([[], 'a', 'b']);
        });
        ...
        it("covers additional pushWithPath edge cases", () => {
            ...
            const data = [[]];
            expect(Path.pushWithPath(data, "invalid.path", 'value')).toEqual(data);
    ...
    describe("undotExpand", () => {
        ...
        it("handles empty or null input", () => {
            ...
            expect(Path.undotExpand(null as unknown as Record<string, unknown>)).toEqual([]);
        });
        ...
        it("handles non-string keys", () => {
            ...
            expect(Path.undotExpand(input as Record<string, unknown>)).toEqual(['a', 'b']);
        });
    ...
    describe("getNestedValue", () => {
        ...
        it("returns undefined for non-existent paths", () => {
            expect(Path.getNestedValue(testData, "0.nonexistent")).toBeUndefined();
    ...
    describe("getMixedValue", () => {
        ...
        it("resolves default value for non-accessible data", () => {
            expect(Path.getMixedValue("not-array", 0, 'default')).toBe('default');
            expect(Path.getMixedValue(null, 0, 'default')).toBe('default');
        });

        it("gets values by numeric key", () => {
            expect(Path.getMixedValue(testData, 0)).toEqual({ name: 'John', items: ['a', 'b'] });
        ...
        it("calls default function when value not found", () => {
            ...
            expect(Path.getMixedValue(testData, "nonexistent", defaultFn)).toBe('computed-default');
        });

        it("returns default for non-existent mixed paths", () => {
            expect(Path.getMixedValue(testData, "0.nonexistent", 'default')).toBe('default');
        });

        it("covers getMixedValue edge cases", () => {
            // Test line 706: When data is not accessible for simple keys without dots
            expect(Path.getMixedValue("not-array", "simple", 'default')).toBe('default');
            
            // Test line 723: When data is not accessible for all-numeric paths
            expect(Path.getMixedValue(null, "1.2.3", 'default')).toBe('default');
        });
    });
    ...
    describe("pushMixed", () => {
        ...
        it("returns new array with values when data is not array and key is null", () => {
            expect(Path.pushMixed("not-array", null, 'a', 'b')).toEqual(['a', 'b']);
        });
    ...
    describe("setMixedImmutable", () => {
        it("returns value as array when key is null", () => {
            expect(Path.setMixedImmutable(['a'], null, 'replacement')).toEqual('replacement');
            expect(Path.setMixedImmutable(['a'], undefined, 'replacement')).toEqual('replacement');
        });
        ...
        it("covers additional edge cases for maximum coverage", () => {
            ...
            const result3 = Path.setMixedImmutable(complexData, "0.nested.new", 'added');
            expect(result3[0]).toEqual({ nested: { deep: 'value', new: 'added' } });
        ...
        it("covers pushWithPath specific error conditions", () => {
            ...
            }).toThrow('Array value for key [0.1] must be an array, object found.');
            
            // Test lines 484-487: pushWithPath boolean error
            const data3 = [true]; // Boolean at index 0
            expect(() => {
                Path.pushWithPath(data3, "0.1", 'value');
            }).toThrow('Array value for key [0.1] must be an array, boolean found.');
        });
        ...
        it("covers remaining uncovered lines for 100% coverage", () => {
            ...
            expect(result4[0]).toEqual({ obj: { nested: 'value', new: 'added' } });
