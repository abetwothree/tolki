export class Trimmer {
     ...
     */
    static readonly INVISIBLE_CHAR_CLASS: string = (() => {
        const cps = [
    ...
    private static get defaultClass(): string {
        // JS \s covers standard whitespace; add invisible class and explicit NUL (\u0000)
        return `\\s${Trimmer.INVISIBLE_CHAR_CLASS}\\u0000`;
    }

    private static escapeForClass(s: string): string {
        return s.replace(/[-\\^$*+?.()|[\]{}]/g, "\\$&");
    }

    private static computeIndents(value: string): {
        baseIndent: number;
        tailIndent: number;
    } {
        let baseIndent = 0;
    ...
    static trim(value: string, charlist: string | null = null): string {
        if (charlist == null || charlist === "") {
            const { baseIndent, tailIndent } = Trimmer.computeIndents(value);

            const cls = Trimmer.defaultClass;
        ...
        const re = new RegExp(
            `^[${Trimmer.escapeForClass(charlist)}]+|[${Trimmer.escapeForClass(charlist)}]+$`,
            "gu",
        );
    ...
    static ltrim(value: string, charlist: string | null = null): string {
        ...
        const re = new RegExp(`^[${Trimmer.escapeForClass(charlist)}]+`, "gu");
        return value.replace(re, "");
    }
    ...
    static rtrim(value: string, charlist: string | null = null): string {
        if (charlist == null || charlist === "") {
            ...
            if (out.includes("\n")) {
                const { baseIndent, tailIndent } =
                    Trimmer.computeIndents(value);
