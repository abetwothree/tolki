...
- `phpTypeName(value)` — a value's type name as PHP's `gettype()` renders it: `"NULL"`, `"integer"` for an integral number, `"double"` for a non-integral one, and the JavaScript `typeof` otherwise. Deliberately **not** the same function as `typeOf`, which answers `"object"` for `null`, `"array"` for an array, and `"number"` for every number; both spellings appear in Laravel-parity error messages and neither can stand in for the other.
