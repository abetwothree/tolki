...
Add four helpers, widen one existing signature, and correct two existing helpers' behaviour to match PHP.
...
- `arrayableValues(items)` now unwraps an `all()`/`toArray()`/`toJSON()`-bearing object, a `Map`, or any other iterable before reading its values, matching `Arr::from`'s `getArrayableItems`. It previously read such an object's own enumerable fields instead of its contents — for example, a `Map`-like object's own methods rather than its entries.
...
Code relying on the previous JavaScript-shaped semantics of `arrayableValues` or `compareValues` will observe different results.
